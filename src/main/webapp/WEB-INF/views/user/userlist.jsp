...
<form name="findForm" id="findForm" action="<%=basePath %>user/userlist" method="get" style="margin-top: 20px; margin-left: 29px; color: #666666;">
...
<table class="list" border="1" bordercolor="#ccc" cellspacing="10" cellpadding="10">
	...
	<c:forEach items="${userList.content}" var="list" varStatus="status">
		<tr> 
			...
			<td>${list.todaymoney}</td>
			<td>
				<a style="color:#54bc2d;" href="<%=basePath%>user/userdetail?id=${list.id}">详情</a>
				<a style="color:#ff6600;" href="<%=basePath%>user/edit_marketing_condition?id=${list.id}">营销条件</a>
			</td>
		</tr>	
		</c:forEach>
</table>
...
<div class="pagestart">
	...
	<div class="pageinfo">
		...
		<button
			onclick="gotopage('<%=basePath%>user/userlist', '${userList.totalpages }')"
			<c:if test="${userList.pageno == userList.totalpages}"> class="button3"  disabled</c:if>

...
<div class="navigation">当前位置：（管理员）订单审核管理&nbsp;>&nbsp;订单审核&nbsp;>&nbsp;<a href="javascript: history.go(-1);">返回</a></div>
...
<!-- 			审核中-审核中 -->
			<c:if test="${clist.checkstatus == 302}">
					...
					<button class="button border-other button-little" onclick="publish('${fn:substring(clist.articleContents, 0, 2)}',${clist.id});">发布</button>
					<a class="button border-yellow button-little" href="<%=basePath%>content/reject?id=${clist.id}" onclick="{if(confirm('确认驳回吗?')){return true;}else return false;}">驳回</a>
...
<div class="pagestart">
	...
	<div class="pageinfo">
		<button
			onclick="gotopage('<%=basePath%>content/admin_list', '1')"
			<c:if test="${list.pageno == 1}"> class="button3"  disabled</c:if>
			<c:if test="${list.pageno > 1}"> class="button3"</c:if>>首页</button>
		<button
			onclick="gotopage('<%=basePath%>content/admin_list', '${list.pageno-1 }')"
			<c:if test="${list.pageno == 1}"> class="button3"  disabled</c:if>
		...
		<button
			onclick="gotopage('<%=basePath%>content/admin_list', '${list.totalpages }')"
			<c:if test="${list.totalpages==0}"> class="button3"  disabled</c:if>
			<c:if test="${list.pageno == list.totalpages}"> class="button3"  disabled</c:if>

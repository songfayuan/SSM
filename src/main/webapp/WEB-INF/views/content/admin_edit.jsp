...
$(document).ready(function()
{
	  editor=UE.getEditor('articleContents');
...
function gotoUrl(state){
	...
	var first_province = $("#province").val();
...
//	 	alert(cityname);
	...
	var articleContents=editor.getContent();
	$('#articleContents').val(articleContents);
    ...
    if(first_province==null || first_province=="" || first_province==undefined){
    	alert("省份、市、自治州不能为空！");
    	return;
    }
    if(provincename==null || provincename=="" || provincename==undefined){
    ...
    if(articleContents==null || articleContents=="" || articleContents==undefined){
    	alert("文章内容不能为空！");
    	return;
    }
		...
		$.ajax(
		{
			...
			success:function(data)
            {
				if(data=="yes"){
					...
					activity.action='<%=basePath%>content/admin_add';
				   	activity.submit();
				}
				if(data=="no"){
...
<%-- 	<c:forEach items="${list}" var="list">
		...
		<div class="itempanel">
			...
			<div class="floatleft editor">
				<input type="hidden" name="headimg" id="img" value="${content.headimg }"/><div style=" color: red; margin-top: 5px; float: right; ">*</div>
		...
		<div class="itempanel">
			<div class="floatleft title">文章标题：</div>
			<div class="floatleft editor"><input type="text" id="articleTitle"  style="border: 2px solid #ff6600;" name="articleTitle" value="${content.articleTitle }" placeholder='请输入文章标题...'/><div style=" color: red; margin-top: 5px; float: right; ">*</div></div>
			<div class="floatleft message">请输入文章标题！</div>
			<div class="clear"></div>
		</div>
		
		<div class="itempanel">
			<div class="floatleft title">文章内容：</div>
			<div class="floatleft editor">
			    <script type="text/plain" id="articleContents" name="articleContents" style="width:600px;">${content.articleContents}</script>
			</div>
			<div class="floatleft message">请在此输入文章内容！</div>
			<div class="clear"></div>
		</div>
		...
		<div class="itempanel">
			...
			<div class="floatleft editor">
				<input type="hidden" name="bottomimg" id="img2" value="${content.bottomimg }"/><div style=" color: red; margin-top: 5px; float: right; ">*</div>
...
<%-- 		<input type="hidden" id="edittime" name="edittime" value="${content.edittime }"/> --%>
		...
		<div class="actionbar">
		    <input id="submit_btn"  class="greenbutton2" style="margin-left: 55px;width: 97px;" onclick="gotoUrl(1)" type="button" value="提交发布"/>
